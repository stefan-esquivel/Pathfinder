...
bool set_motor(int target)
{
	...
	//change: initalize Varables
	int acc = 4;
	int maxAmp = 69;
	//

	//change: inssures the motor dosent go over the maximum power
	if(abs(delta) > maxAmp)
	{
		// change: takes the unit direction of delta and multiplys it by the maxAmp. Slows the motor power down when it is close to the target.
		delta = sgn(delta) * maxAmp;
	}
	//change: turns off the motor if the sensor reads the threashold accuracy
	if(abs(delta) <= acc)
	{
		//change: sets stops the motor when it is in the threashold
		motor[motor1] = 0;
		return true;
	}
	else
	{
		//change: assigns the motor to delta
		motor[motor1] = delta;
		return false;
	}
	//
