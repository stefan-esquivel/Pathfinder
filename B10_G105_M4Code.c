...
static const int ARM_MAX_POSITION_SENSOR_VALUE = 3400;
...
task targetCheck()
{
	while(true)
	{
		...
		if(SensorValue[frontSonar] < 9 && SensorValue[frontSonar] != -1)
		{
	...
	while(true)
	{
		...
		if(SensorValue[rightButton])
		{
		  ...
		  wait1Msec(100);
			...
			setDriveTrainEncoderTargets(0, 2500, true);
			//wait until it finishes traveling or at the target
			waitUntil(SensorValue[leftDTSensor] > 2490 || atTarget);
			//waitUntil(false);
			//stop the robot
			setDriveTrainEncoderTargets(0,0,true);
			...
			setDriveTrainEncoderTargets(0, 500, true);
			//wait until it finishes traveling or at the target
			waitUntil(SensorValue[leftDTSensor] > 490 || atTarget);
			//stop the robot
			setDriveTrainEncoderTargets(0,0,true);
			...
			while(!atTarget)
			{
				...
				setDriveTrainEncoderTargets(0, 200, true);
				//wait until it finishes traveling or at the target
				waitUntil(SensorValue[leftDTSensor] > 190 || atTarget);
				//stop the robot
				setDriveTrainEncoderTargets(0,0,true);
			...
			waitUntil(SensorValue[armPotentiometer] > 3350);
			wait1Msec(300);
			//move arm back to not touch the cable
			armEncoderTarget = ARM_MIN_POSITION_SENSOR_VALUE;
			waitUntil(SensorValue[armPotentiometer] < 1400);
			wait1Msec(100);
			//back up robot
		...
		if(SensorValue[touch])
		{
			...
			pointedAtTarget = false;
				...
				wait1Msec(200);
				//move robot with step 100
				setDriveTrainEncoderTargets(0, 2000, true);
				//wait until it finishes traveling or at the target
				waitUntil(SensorValue[leftDTSensor] > 1990 || atTarget);
				wait1Msec(100);
				//stop the robot
				setDriveTrainEncoderTargets(0,0,true);
				delta = 1;
