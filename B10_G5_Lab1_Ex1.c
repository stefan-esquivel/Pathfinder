...
task main()
{
    ...
    // Initialization here.
	  ...
	  int result;
		...
		while(true)
		{
			//change: initates the current_meas_point = (max_v + min_v) / 2 and result
			...
			if(result == 0)
			{
				// If current_meas_point measurement indicates equality, break out of loop.
				break;
		  }
		  //change: added new condition when result = 1
		  else if(result == 1)
		  {
		  	//change: move current max down from meas by 1.
		  	max_v = current_meas_point - 1;
		  }
		  else
		  {
				//change: move current min up from meas by 1.
